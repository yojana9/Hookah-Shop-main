...
const normalizeBadge = (b) =>
  typeof b === "string"
    ...
    : b && typeof b === "object"
    ? { text: b.text ?? b.label ?? "", variant: b.variant ?? inferVariant(b.text ?? b.label ?? "") }
    : null;
  ...
  return (
    <div className="group flex flex-col items-start relative">
        ...
        {pillList.length > 0 && (
          <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
            {pillList.map((b, i) => (
              <span
                key={`${id}-badge-${i}`}
                className={`inline-flex items-center rounded-full px-3 py-1 text-[10px] font-extrabold uppercase tracking-wide shadow-sm ${BADGE_STYLES[b.variant] ?? BADGE_STYLES.purple}`}
              >
                {b.text}
              </span>
            ))}
      ...
      <div className="w-full mt-3">
        <strong className="text-black text-lg sm:text-xl block">{title}</strong>
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 lg:px-4 py-10">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[40px] leading-none font-extrabold tracking-tight uppercase">Just Arrived</h2>
